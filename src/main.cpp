...
#if defined(NDEBUG)
# error "Arthasiglos cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2d7f83cf158df4e43c19618ec66061ff61dd555b3517b9df2a6e2c3cb16428c2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Arthasiglos: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Arthasiglos Signed Message:\n";
...
{
    // Arthasiglos: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 33000); // Arthasiglos: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Arthasiglos: 3 days
static const int64 nTargetSpacing = 2 * 60; // Arthasiglos: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Arthasiglos: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Arthasiglos: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7ab9a265dbee1ee26b1f1e32335e484a080efff3d19871b807b74d8a175563b6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 25/Apr/2022 Elon Musk and Twitter Reach Deal for Sale";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d2719d77fe73c5398c59186a3ea18d986c7bdc8d3f34e412e2fb4ca93acc25e591e2bc9b72a9818fff9f743807541f35660679d3f7bab9afef0ea041893c307c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1651669100;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085409550;

        if (fTestNet)
        {
            block.nTime    = 1651669038;
            block.nNonce   = 385292921;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd8d14f024f37bea2922545ea01c098d05d6f4e0f5ab6f13566a4feecf8bf06bf"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Arthasiglos: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArthasiglosMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArthasiglosMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ArthasiglosMiner(CWallet *pwallet)
{
    printf("ArthasiglosMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("arthasiglos-miner");
    ...
    try { loop {
        ...
        printf("Running ArthasiglosMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ArthasiglosMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ArthasiglosMiner, pwallet));
}
